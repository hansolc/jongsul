...
def new_alarm():
    ...
    def mkdate():
        ...
        #os.system('py synthesizer.py --load_path logs/son --text="%s"' % result_data[j])\
        
        #client server
        
        data_transferred = 0
        filename = result_data[j]
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            #파일 저장 경로 설정 알람 파일에 있어서 수정 필요
            #여기에 너가 서버에서 부터 음성 파일 받을 곳 지정해줘
            #alarm_num 은 내가 전역변수로 써서 아마 안바꿔 주고 경로만 바꿔주면 될듯.
            with open('/home/hs/jongsul/work/alarm'+str(alarm_num)+'.manual.wav', 'wb') as f:
                try:
                    while data:
                        f.write(data)
